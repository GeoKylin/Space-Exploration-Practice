...
n=length(igsg1)/5;
for i=0:n-1
    d=[igsg1(5*i+1,1:16) igsg1(5*i+2,1:16) igsg1(5*i+3,1:16) ...
        igsg1(5*i+4,1:16) igsg1(5*i+5,1:9)];
    data(i+1,:)=interp(d,2);
end
data=data/10;
load coast;
...
title('GLOBAL IONOSPHERE MAPS FOR DAY 001, 2017 - 02:00 UT');
set(gca,'XTick',-180:45:180, 'YTick',-60:30:60);
axis equal; axis([-180 180 -87.5 87.5]);
% colorbar([-180 -60 10 10]);
